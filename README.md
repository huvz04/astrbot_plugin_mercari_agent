...
插件的数据目录中会创建 SQLite 数据库（任务、Attempt、Listing、处理记录和通知
记录）以及 Chroma 持久化文件（由本地 Markdown 知识构建）。Listing 标题、描述、
价格、URL 与通知文本会进入本地数据库；请按自己的数据保留策略保护或清理该目录。
错误摘要会做敏感字段脱敏，采集响应正文不会写入数据库。日志不应包含凭据、Cookie
或访问令牌；如需排障，也请先移除敏感数据。
